...
matched$city_pop = as.numeric(gsub(",","", matched$city_pop))
matched$metro_pop = as.numeric(gsub(",","", matched$metro_pop))

ggplot(matched, aes(x = city_pop, y = metro_pop)) + geom_point()

write.csv(matched, "data/city_metro_pops.csv")